...
type (
	...
	ISysAdminUserAtch interface {
		// Model 授权/密钥ORM模型
		Model(ctx context.Context, option ...*handler.Option) *gdb.Model
		// List 获取授权/密钥列表
		List(ctx context.Context, in *sysin.AdminUserAtchListInp) (list []*sysin.AdminUserAtchListModel, totalCount int, err error)
		// Edit 修改/新增授权/密钥
		Edit(ctx context.Context, in *sysin.AdminUserAtchEditInp) (err error)
		// Delete 删除授权/密钥
		Delete(ctx context.Context, in *sysin.AdminUserAtchDeleteInp) (err error)
		// View 获取授权/密钥指定信息
		View(ctx context.Context, in *sysin.AdminUserAtchViewInp) (res *sysin.AdminUserAtchViewModel, err error)
	}
...
var (
	...
	localSysAdminUserAtch ISysAdminUserAtch
...
func SysAdminUserAtch() ISysAdminUserAtch {
	if localSysAdminUserAtch == nil {
		panic("implement not found for interface ISysAdminUserAtch, forgot register?")
	}
	return localSysAdminUserAtch
}

func RegisterSysAdminUserAtch(i ISysAdminUserAtch) {
	localSysAdminUserAtch = i
}
